...
#include <fly_to_learn/simulator/operations_cpu.h>
#include <fly_to_learn/simulator/ui.h>
...
class websocket_session : public std::enable_shared_from_this<websocket_session> 
{
    ...
    using ABLATION_SPEC = fly_to_learn::config::DEFAULT_ABLATION_SPEC;
    struct CONFIG: fly_to_learn::config::Config<ABLATION_SPEC>
    {
        using DEV_SPEC = rlt::devices::cpu::Specification<rlt::devices::math::CPU, 
                                                          rlt::devices::random::CPU, 
                                                          rlt::devices::logging::CPU>;
    ...
    using TI = CONFIG::TI;
    fly_to_learn::TrainingState<CONFIG> ts;
...
//        );
    ...
    void refresh()
    {
        ...
        // transfers trajectories from a shared queue to an ongoing trajectory list up 100
        // counts the # of new trajectories
        {
        ...
        // If there are new trajectories, assign them to idle drones or creates new drones
...
//                std::cout << "Adding drone at " << ui << std::endl;
        ...
        // Sends the state of the drones to the clients and removes the drones that have finished their trajectories
    ...
    void on_read(beast::error_code ec, std::size_t bytes_transferred) 
    {
        ...
        if(message["channel"] == "startTraining"){
            ...
            this->t = std::thread([seed, this](){
                ...
                fly_to_learn::init(this->ts, seed);
                for(TI step_i=0; step_i < CONFIG::STEP_LIMIT; step_i++){
                    fly_to_learn::step(this->ts);
                }
...
private:
    ...
    void create_response()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                << " (you might need to run \"get_dependencies.sh\""
                << " to download the UI dependencies into the static folder)"
                << std::endl;
            }
        }
    }
    void maybe_upgrade() 
    {
        if (beast::websocket::is_upgrade(request_)) 
        {
            ...
            std::make_shared<websocket_session>(std::move(socket_))->run(std::move(request_)); // Key part of the training
            return;
        }
    }
...
{
    std::cout << "Note: This executable should be executed \
    in the context (working directory) of the main repo \
    e.g. ./build/src/rl_environments_multirotor_ui 0.0.0.0 8000" << std::endl;
    try
    {
        ...
        if(argc != 3)
        {
            ...
            (e.g. \'0.0.0.0 8000\' for localhost 8000)" << std::endl;
            return EXIT_FAILURE;
        }
        ...
        std::cout << "Web interface coming up at : http://" \
