...
class Drone{
  constructor(model, origin, envParams, displayIMUCoordinateSystem, displayActions){
    ...
    const centerSize = model.mass / 10
    const centerForm = new THREE.BoxGeometry(centerSize * 1.0, centerSize * 0.6, centerSize*0.1) // [NOTE] change body size
    ...
    for(const [rotorIndex, rotor] of model.rotors.entries()){
      ...
      additionalRotation.setFromAxisAngle(new THREE.Vector3(1, 0, 0), Math.PI / 2); // 90 degrees
      rotorGroup.quaternion.multiply(additionalRotation);
    ...
    // [Note] axle
    if (this.rotorGroups.length >= 4) {
      const axleMaterial = new THREE.MeshBasicMaterial({ color: 0x000000 }); // Black axle

      // Create axles between the front and rear rotors (assuming index 0 & 3 are front, and 1 & 2 are rear)
      this.createAxleBetweenRotors(this.rotorGroups[0], this.rotorGroups[3], axleMaterial);
      this.createAxleBetweenRotors(this.rotorGroups[1], this.rotorGroups[2], axleMaterial);
    }
  }
  //[Note] axle
  createAxleBetweenRotors(rotorGroup1, rotorGroup2, material) {
    const distance = rotorGroup1.position.distanceTo(rotorGroup2.position);
    const axleGeometry = new THREE.CylinderGeometry(0.02, 0.02, distance, 32);
    const axle = new THREE.Mesh(axleGeometry, material);
    axle.position.copy(rotorGroup1.position).lerp(rotorGroup2.position, 0.5);
    axle.lookAt(rotorGroup2.position);
    axle.rotateX(Math.PI / 2); // Align horizontally
    this.drone.add(axle);
  }
  get(){
