#ifndef FLY_TO_LEARN_SIMULATOR_PARAMETERS_REWARD_FUNCTIONS_SQUARED_H
#define FLY_TO_LEARN_SIMULATOR_PARAMETERS_REWARD_FUNCTIONS_SQUARED_H
...
namespace rl_tools::rl::environments::multirotor::parameters::reward_functions
{
    ...
    typename rl::environments::multirotor::parameters::reward_functions::Squared<T>::Components 
        reward_components(DEVICE& device, const rl::environments::Multirotor<SPEC>& env,
                          const rl::environments::multirotor::parameters::reward_functions::Squared<T>& params,
                          ...
                          RNG& rng)
    {
        ...
        for(TI i = 0; i < ACTION_DIM; i++){
            action_diff[i] = get(action, 0, i) - params.action_baseline; //rl_tools/include/rl_tools/containers/operations_generic.h
        }
    ...
    template<typename DEVICE, typename SPEC, typename ACTION_SPEC, typename T, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT void log_reward(DEVICE& device, const rl::environments::Multirotor<SPEC>& env, \
                                                const rl::environments::multirotor::parameters::reward_functions::Squared<T>& params, \
                                                const typename rl::environments::Multirotor<SPEC>::State& state, \
                                                const Matrix<ACTION_SPEC>& action,  \
                                                const typename rl::environments::Multirotor<SPEC>::State& next_state, RNG& rng) 
    {
    ...
    template<typename DEVICE, typename SPEC, typename ACTION_SPEC, typename T, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT typename SPEC::T reward(DEVICE& device, \
