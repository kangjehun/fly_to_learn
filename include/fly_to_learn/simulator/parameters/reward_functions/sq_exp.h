#ifndef FLY_TO_LEARN_SIMULATOR_PARAMETERS_REWARD_FUNCTIONS_SQ_EXP_H
#define FLY_TO_LEARN_SIMULATOR_PARAMETERS_REWARD_FUNCTIONS_SQ_EXP_H
...
namespace rl_tools::rl::environments::multirotor::parameters::reward_functions
{
    ...
    template <typename DEVICE, typename SPEC, typename T, typename T_STATE, typename TI_STATE,
              typename LATENT_STATE, typename ACTION_SPEC, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT static typename SPEC::T
    reward(DEVICE &device,
           const rl::environments::Multirotor<SPEC> &env,
           const rl::environments::multirotor::parameters::reward_functions::SqExp<T> &params,
           const rl::environments::multirotor::StateBase<T_STATE, TI_STATE> &state,
           const Matrix<ACTION_SPEC> &action,
           const rl::environments::multirotor::StateBase<T_STATE, TI_STATE> &next_state,
           RNG &rng, bool log_components = true)
    {
        ...
        T orientation_cost = 1 - state.orientation[0] * state.orientation[0]; 
        // math::sq(device.math, 2 * math::acos(device.math, quaternion_w));
        ...
        T weighted_sq_cost = params.position * position_cost + 
    ...
    reward(DEVICE &device,
           const rl::environments::Multirotor<SPEC> &env,
           const rl::environments::multirotor::parameters::reward_functions::SqExpMultiModal<T, TI, N_MODES> &params,
