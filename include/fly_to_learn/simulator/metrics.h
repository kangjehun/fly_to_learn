...
namespace rl_tools
{
    ...
    namespace rl::utils::validation::metrics::multirotor
    {
        ...
        typename STATE::T get_quantity(DEVICE& device, 
                                       rl::utils::validation::metrics::multirotor::MultirotorStateQuantity quantity, 
                                       STATE& state, STATE& next_state, typename STATE::T dt)
        {
    ...
    template <typename DEVICE, typename SPEC, 
              enum rl::utils::validation::metrics::multirotor::MultirotorStateQuantity T_QUANTITY, 
              typename T_TI, T_TI T_START_STEP>
        ...
        for(TI episode_i = 0; episode_i < SPEC::N_EPISODES; episode_i++)
        {
            ...
            if(task.episode_length[episode_i] == SPEC::MAX_EPISODE_LENGTH)
            {
                ...
                for(TI step_i = T_START_STEP; step_i < task.episode_length[episode_i]; step_i++)
                {
                    ...
                    T error = rl::utils::validation::metrics::multirotor::get_quantity(
                        device, 
                        T_QUANTITY, 
                        state, next_state, 
                        task.environment[0].parameters.integration.dt);
                    if(error > max_error)
                    {
                        max_error = error;
                    }
    ...
    template <typename DEVICE, 
              ...
              typename T_TI, 
              T_TI T_START_STEP>
    typename SPEC::T evaluate(
        DEVICE& device, 
        rl::utils::validation::metrics::MaxErrorStd<T_QUANTITY, T_TI, T_START_STEP>, 
        rl::utils::validation::Task<SPEC>& task)
    {
        utils::assert_exit(device, task.completed, "Task is not completed");
        using TI = typename SPEC::TI;
        using T = typename SPEC::T;
        ...
        TI max_pos_error_count = 0;
        T mean = evaluate(device, 
                          rl::utils::validation::metrics::MaxErrorMean<T_QUANTITY, T_TI, T_START_STEP>{}, 
                          task);
        for(TI episode_i = 0; episode_i < SPEC::N_EPISODES; episode_i++)
        {
            auto& eb = task.episode_buffer[episode_i];
