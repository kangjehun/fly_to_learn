#ifndef FLY_TO_LEARN_SIMULATOR_QUATERNION_HELPER_H
#define FLY_TO_LEARN_SIMULATOR_QUATERNION_HELPER_H

#include <rl_tools/utils/generic/vector_operations.h>

namespace rl_tools::rl::environments::multirotor{
    // @func : calculate the derivative of a quaternion based on the current orientation and the angular velocity
    template <typename DEVICE, typename T>
    void quaternion_derivative(const T q[4], const T omega[3], T q_dot[4]) 
    {
        // FLOPS: 3 (MAC) * 4 + 4 = 16
    ...
    // @func : applies a rotation represented by a quaternion to a vector
    template <typename DEVICE, typename T>
