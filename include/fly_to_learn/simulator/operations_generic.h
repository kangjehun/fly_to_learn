#ifndef FLY_TO_LEARN_SIMULATOR_OPERATIONS_GENERIC_H
#define FLY_TO_LEARN_SIMULATOR_OPERATIONS_GENERIC_H
...
namespace rl_tools{
    ...
    RL_TOOLS_FUNCTION_PLACEMENT static void scalar_multiply(DEVICE& device, 
                                                            const typename rl::environments::multirotor::StateBase<T, TI>& state, 
                                                            T2 scalar, 
                                                            typename rl::environments::multirotor::StateBase<T, TI>& out)
    {
        for(int i = 0; i < 3; ++i){
    ...
    RL_TOOLS_FUNCTION_PLACEMENT static void scalar_multiply(DEVICE& device, 
                                                            const typename rl::environments::multirotor::StateRotors<T, TI, NEXT_COMPONENT>& state, 
                                                            T2 scalar, 
                                                            typename rl::environments::multirotor::StateRotors<T, TI, NEXT_COMPONENT>& out)
    {
    ...
    RL_TOOLS_FUNCTION_PLACEMENT static void scalar_multiply(DEVICE& device, 
                                                            ...
                                                            T2 scalar, 
                                                            STATE& out, 
                                                            utils::typing::enable_if_t<!STATE::REQUIRES_INTEGRATION, bool> disable = false)
    {
    ...
    template<typename DEVICE, typename T, typename TI, typename T2>
    RL_TOOLS_FUNCTION_PLACEMENT static void scalar_multiply(DEVICE& device, 
                                                            typename rl::environments::multirotor::StateBase<T, TI>& state, 
                                                            T2 scalar)
    {
    ...
    template<typename DEVICE, typename T, typename TI, typename T2, typename NEXT_COMPONENT>
    RL_TOOLS_FUNCTION_PLACEMENT static void scalar_multiply(DEVICE& device, 
                                                            typename rl::environments::multirotor::StateRotors<T, TI, NEXT_COMPONENT>& state, 
                                                            T2 scalar)
    {
        scalar_multiply(device, state, scalar, state);
    }
    ...
    template<typename DEVICE, typename T, typename TI, typename T2>
    RL_TOOLS_FUNCTION_PLACEMENT static void scalar_multiply_accumulate(DEVICE& device, 
                                                                       const typename rl::environments::multirotor::StateBase<T, TI>& state, 
                                                                       T2 scalar, 
                                                                       typename rl::environments::multirotor::StateBase<T, TI>& out)
    {
    ...
    static void scalar_multiply_accumulate(DEVICE& device, 
                                           const typename rl::environments::multirotor::StateRotors<T, TI, NEXT_COMPONENT>& state, 
                                           T2 scalar, 
                                           typename rl::environments::multirotor::StateRotors<T, TI, NEXT_COMPONENT>& out)
    {
    ...
    {
        static_assert(!STATE::REQUIRES_INTEGRATION);
        scalar_multiply_accumulate(device, 
                                   static_cast<const typename STATE::NEXT_COMPONENT&>(state), 
                                   scalar, 
                                   static_cast<typename STATE::NEXT_COMPONENT&>(out));
    }

    // add
    template<typename DEVICE, typename T, typename TI>
    RL_TOOLS_FUNCTION_PLACEMENT static void add_accumulate(DEVICE& device, 
                                                           const typename rl::environments::multirotor::StateBase<T, TI>& s1, 
                                                           const typename rl::environments::multirotor::StateBase<T, TI>& s2, 
                                                           typename rl::environments::multirotor::StateBase<T, TI>& out)
    {
        for(int i = 0; i < 3; ++i){
    ...
    RL_TOOLS_FUNCTION_PLACEMENT static void add_accumulate(DEVICE& device, 
                                                           const typename rl::environments::multirotor::StateRotors<T, TI, NEXT_COMPONENT>& s1, 
                                                           const typename rl::environments::multirotor::StateRotors<T, TI, NEXT_COMPONENT>& s2, 
                                                           typename rl::environments::multirotor::StateRotors<T, TI, NEXT_COMPONENT>& out)
    {
    ...
    {
        static_assert(!STATE::REQUIRES_INTEGRATION);
        add_accumulate(device, 
                       static_cast<const typename STATE::NEXT_COMPONENT&>(s1), 
                       static_cast<const typename STATE::NEXT_COMPONENT&>(s2), 
                       static_cast<typename STATE::NEXT_COMPONENT&>(out));
    }

    // add accumulate with self
    template<typename DEVICE, typename T, typename TI>
    RL_TOOLS_FUNCTION_PLACEMENT static void add_accumulate(DEVICE& device, 
                                                           const typename rl::environments::multirotor::StateBase<T, TI>& s, 
                                                           typename rl::environments::multirotor::StateBase<T, TI>& out)
    {
    ...
    template<typename DEVICE, typename T, typename TI, typename NEXT_COMPONENT>
    RL_TOOLS_FUNCTION_PLACEMENT static void add_accumulate(DEVICE& device, 
                                                           const typename rl::environments::multirotor::StateRotors<T, TI, NEXT_COMPONENT>& s, 
                                                           typename rl::environments::multirotor::StateRotors<T, TI, NEXT_COMPONENT>& out)
    {
        add_accumulate(device, s, out, out);
    }
                                                           ...
                                                           utils::typing::enable_if_t<!STATE::REQUIRES_INTEGRATION, bool> disable = false)
    {
        static_assert(!STATE::REQUIRES_INTEGRATION);
        add_accumulate(device, 
                       static_cast<const typename STATE::NEXT_COMPONENT&>(s), 
                       static_cast<const typename STATE::NEXT_COMPONENT&>(out), 
                       static_cast<typename STATE::NEXT_COMPONENT&>(out));
    }
}
...
namespace rl_tools::rl::environments::multirotor {
    ...
    RL_TOOLS_FUNCTION_PLACEMENT 
    void multirotor_dynamics(DEVICE& device, 
                             ...
                             StateBase<T, TI>& state_change) 
    {
        ...
        for(typename DEVICE::index_t i_rotor = 0; i_rotor < 4; i_rotor++){
            ...
            T thrust_magnitude = params.dynamics.thrust_constants[0] + 
                                 params.dynamics.thrust_constants[1] * rpm + 
                                 params.dynamics.thrust_constants[2] * rpm * rpm;
            T rotor_thrust[3];
            // force due to the thrust (rotating frame)
            utils::vector_operations::scalar_multiply<DEVICE, T, 3>(params.dynamics.rotor_thrust_directions[i_rotor], 
                                                                    thrust_magnitude, 
                                                                    rotor_thrust); 
            utils::vector_operations::add_accumulate<DEVICE, T, 3>(rotor_thrust, thrust);
            // torque due to the thrust (rotating frame)
            utils::vector_operations::scalar_multiply_accumulate<DEVICE, T, 3>(params.dynamics.rotor_torque_directions[i_rotor], 
                                                                               thrust_magnitude * params.dynamics.torque_constant,
                                                                               torque);
        ...
        //                          params.dynamics.thrust_constants[2] * rpm * rpm;
        //     T rotor_thrust[3];
        //     // force due to the thrust (intertial frame)
        //     utils::vector_operations::scalar_multiply<DEVICE, T, 3>(params.dynamics.rotor_thrust_directions[i_rotor], 
        //                                                             thrust_magnitude, 
        //                                                             rotor_thrust); 
        //     utils::vector_operations::add_accumulate<DEVICE, T, 3>(rotor_thrust, thrust);
        //     // torque due to the thrust (inertial frame)
        //     utils::vector_operations::scalar_multiply_accumulate<DEVICE, T, 3>(params.dynamics.rotor_torque_directions[i_rotor], 
        //                                                                        thrust_magnitude * params.dynamics.torque_constant,
        //                                                                        torque);
        //     utils::vector_operations::cross_product_accumulate<DEVICE, T>(params.dynamics.rotor_positions[i_rotor], rotor_thrust, torque);
        ...
        // // linear_velocity (inertial frame : quaternion)
        // state_change.position[0] = state.linear_velocity[0];
        // state_change.position[1] = state.linear_velocity[1];                                                                
        // state_change.position[2] = state.linear_velocity[2];

        // // angular_velocity (rotating frame)
        // // flops: 16
        // quaternion_derivative<DEVICE, T>(state.orientation, state.angular_velocity, state_change.orientation);
    ...
    RL_TOOLS_FUNCTION_PLACEMENT void multirotor_dynamics(
        DEVICE& device, 
        const PARAMETERS& params, 
        const StateRandomForce<T, TI, NEXT_COMPONENT>& state, 
        const T* action, 
        StateRandomForce<T, TI, NEXT_COMPONENT>& state_change)
    {
    ...
    RL_TOOLS_FUNCTION_PLACEMENT void multirotor_dynamics(
        DEVICE& device, 
        const PARAMETERS& params, 
        const StateRotors<T, TI, NEXT_COMPONENT>& state, 
        const T* action, 
        StateRotors<T, TI, NEXT_COMPONENT>& state_change) 
    {
    ...
    RL_TOOLS_FUNCTION_PLACEMENT void multirotor_dynamics_dispatch(
        DEVICE& device, 
        const PARAMETERS& params, 
        const STATE& state, 
        const T* action, 
        STATE& state_change) 
    {
        // this dispatch function is required to pass the multirotor dynamics function to the integrator (euler, rk4) 
        // as a template parameter (so that it can be inlined/optimized at compile time)
        // If we would try to pass the multirotor_dynamics function directly 
        // the state type-based overloading would make 
        // the inference of the auto template parameter for the dynamics function in the integrator function impossible
...
namespace rl_tools{
    ...
    static void initial_parameters(
        DEVICE& device, 
...
//        printf("initial state: %f %f %f %f %f %f %f %f %f %f %f %f %f\n", state.state[0], state.state[1], state.state[2], state.state[3], state.state[4], state.state[5], state.state[6], state.state[7], state.state[8], state.state[9], state.state[10], state.state[11], state.state[12]);
    ...
    static void initial_state(
        DEVICE& device, 
        rl::environments::Multirotor<SPEC>& env, 
        typename rl::environments::multirotor::StateRandomForce<T, TI, NEXT_COMPONENT>& state)
    {
    ...
    static void initial_state(
        DEVICE& device, 
        rl::environments::Multirotor<SPEC>& env, 
        typename rl::environments::multirotor::StateRotorsHistory<T, TI_H, HISTORY_LENGTH, NEXT_COMPONENT>& state)
    {
        ...
        for(TI step_i = 0; step_i < HISTORY_LENGTH; step_i++)
        {
            ...
            {
                state.action_history[step_i][action_i] = 
                    (state.rpm[action_i] - env.parameters.dynamics.action_limit.min) / (env.parameters.dynamics.action_limit.max - env.parameters.dynamics.action_limit.min) * 2 - 1;
            }
        }
    }
    ...
    RL_TOOLS_FUNCTION_PLACEMENT static void sample_initial_state(
        ...
        rl::environments::Multirotor<SPEC>& env, 
        typename rl::environments::multirotor::StateBase<T, TI>& state, 
        RNG& rng, bool inherited_guidance = false)
    {
        ...
        if(!guidance)
        {
            for(TI i = 0; i < 3; i++)
            {
                state.position[i] = random::uniform_real_distribution(
                    random_dev, 
                    -env.parameters.mdp.init.max_position, 
                    env.parameters.mdp.init.max_position, 
                    rng);
            }
        ...
        for(TI i = 0; i < 3; i++)
        {
            state.linear_velocity[i] = 
                random::uniform_real_distribution(
                    random_dev, 
                    -env.parameters.mdp.init.max_linear_velocity, 
                    env.parameters.mdp.init.max_linear_velocity, 
                    rng);
        }
        for(TI i = 0; i < 3; i++)
        {
            state.angular_velocity[i] = 
                random::uniform_real_distribution(
                    random_dev, 
                    -env.parameters.mdp.init.max_angular_velocity, 
                    env.parameters.mdp.init.max_angular_velocity, 
                    rng);
        }
        initial_parameters(device, env, state);
    ...
    RL_TOOLS_FUNCTION_PLACEMENT static void sample_initial_state(
        DEVICE& device, 
        rl::environments::Multirotor<SPEC>& env, 
        typename rl::environments::multirotor::StateRandomForce<T_S, TI_S, NEXT_COMPONENT>& state, 
        RNG& rng)
    {
        typename DEVICE::SPEC::RANDOM random_dev;
...
//        }
        ...
        RNG& rng)
    {
        ...
        {
            min_rpm = 
                (env.parameters.mdp.init.min_rpm + 1)/2 
                * (env.parameters.dynamics.action_limit.max - env.parameters.dynamics.action_limit.min) 
                + env.parameters.dynamics.action_limit.min;
            max_rpm = 
                (env.parameters.mdp.init.max_rpm + 1)/2 
                * (env.parameters.dynamics.action_limit.max - env.parameters.dynamics.action_limit.min) 
                + env.parameters.dynamics.action_limit.min;
        }
        else{
    ...
    RL_TOOLS_FUNCTION_PLACEMENT static void sample_initial_state(
        DEVICE& device, 
        rl::environments::Multirotor<SPEC>& env, 
        typename rl::environments::multirotor::StateRotorsHistory<T_S, TI_S, HISTORY_LENGTH, NEXT_COMPONENT>& state, 
        RNG& rng)
    {
        using MULTIROTOR = rl::environments::Multirotor<SPEC>;
        using TI = typename DEVICE::index_t;
        ...
        for(TI step_i = 0; step_i < HISTORY_LENGTH; step_i++)
        {
            ...
            {
                state.action_history[step_i][action_i] = 
                    (state.rpm[action_i] - env.parameters.dynamics.action_limit.min) 
                    / (env.parameters.dynamics.action_limit.max - env.parameters.dynamics.action_limit.min) * 2 
                    - 1;
            }
        }
    }
    namespace rl::environments::multirotor
    {
        ...
        RL_TOOLS_FUNCTION_PLACEMENT static void observe(
            ...
            const STATE& state, 
            rl::environments::multirotor::observation::LastComponent<OBSERVATION_TI>, 
        ...
        RL_TOOLS_FUNCTION_PLACEMENT static void observe(
            ...
            rl::environments::multirotor::observation::Position<OBSERVATION_SPEC>, 
            Matrix<OBS_SPEC>& observation, 
            RNG& rng)
        {
            ...
            for(TI i = 0; i < 3; i++)
            {
                ...
                {
                    T noise = random::normal_distribution::sample(
                        typename DEVICE::SPEC::RANDOM{}, 
                        (T)0, 
                        env.parameters.mdp.observation_noise.position, 
                        rng);
                    set(observation, 0, i, state.position[i] + noise);
                }
            }
            auto next_observation = view(
                device, 
                observation, 
                matrix::ViewSpec<1, OBS_SPEC::COLS - OBSERVATION::CURRENT_DIM>{}, 
                0, 
                OBSERVATION::CURRENT_DIM);
        ...
        RL_TOOLS_FUNCTION_PLACEMENT static void observe(
            ...
            const rl::environments::Multirotor<SPEC>& env, 
            const typename rl::environments::Multirotor<SPEC>::State& state, 
            rl::environments::multirotor::observation::OrientationQuaternion<OBSERVATION_SPEC>, 
            Matrix<OBS_SPEC>& observation, 
            RNG& rng)
        {
            using T = typename SPEC::T;
            ...
            for(TI i = 0; i < OBSERVATION::CURRENT_DIM; i++)
            {
                if constexpr(OBSERVATION_SPEC::PRIVILEGED && !SPEC::STATIC_PARAMETERS::PRIVILEGED_OBSERVATION_NOISE)
                {
                    ...
                    T noise = random::normal_distribution::sample(
                        typename DEVICE::SPEC::RANDOM{}, 
                        (T)0, 
                        env.parameters.mdp.observation_noise.orientation, 
                        rng);
        ...
        RL_TOOLS_FUNCTION_PLACEMENT static void observe(
            ...
            const rl::environments::Multirotor<SPEC>& env, 
            const typename rl::environments::Multirotor<SPEC>::State& state, 
            rl::environments::multirotor::observation::OrientationRotationMatrix<OBSERVATION_SPEC>, 
            Matrix<OBS_SPEC>& observation, 
            RNG& rng)
        {
            using T = typename SPEC::T;
            ...
            if constexpr(!OBSERVATION_SPEC::PRIVILEGED || SPEC::STATIC_PARAMETERS::PRIVILEGED_OBSERVATION_NOISE)
            {
                ...
                {
                    T noise;
                    noise = random::normal_distribution::sample(
                        typename DEVICE::SPEC::RANDOM(), 
                        (T)0, 
                        env.parameters.mdp.observation_noise.orientation, 
                        rng);
        ...
        RL_TOOLS_FUNCTION_PLACEMENT static void observe(
            ...
            const rl::environments::Multirotor<SPEC>& env, 
            const typename rl::environments::Multirotor<SPEC>::State& state, 
            rl::environments::multirotor::observation::LinearVelocity<OBSERVATION_SPEC>, 
            Matrix<OBS_SPEC>& observation, 
            RNG& rng)
        {
            using T = typename SPEC::T;
            ...
            for(TI i = 0; i < OBSERVATION::CURRENT_DIM; i++)
            {
                ...
                else
                {
                    T noise = random::normal_distribution::sample(
                        typename DEVICE::SPEC::RANDOM{},
                        (T)0,
                        env.parameters.mdp.observation_noise.linear_velocity,
                        rng);
        ...
        RL_TOOLS_FUNCTION_PLACEMENT static void observe(
            ...
            const rl::environments::Multirotor<SPEC>& env, 
            const typename rl::environments::Multirotor<SPEC>::State& state, 
            rl::environments::multirotor::observation::AngularVelocity<OBSERVATION_SPEC>, 
            Matrix<OBS_SPEC>& observation, 
            RNG& rng)
        {
            using T = typename SPEC::T;
            ...
            for(TI i = 0; i < OBSERVATION::CURRENT_DIM; i++)
            {
                ...
                else
                {
                    T noise = random::normal_distribution::sample(
                        typename DEVICE::SPEC::RANDOM{}, 
                        (T)0, 
                        env.parameters.mdp.observation_noise.angular_velocity, 
                        rng);
        ...
        RL_TOOLS_FUNCTION_PLACEMENT static void observe(
            ...
            const rl::environments::Multirotor<SPEC>& env, 
            const typename rl::environments::Multirotor<SPEC>::State& state, 
            rl::environments::multirotor::observation::RotorSpeeds<OBSERVATION_SPEC>, 
            Matrix<OBS_SPEC>& observation, 
            RNG& rng)
        {
            using T = typename SPEC::T;
        ...
        RL_TOOLS_FUNCTION_PLACEMENT static void observe(
            ...
            const rl::environments::Multirotor<SPEC>& env, 
            const typename rl::environments::Multirotor<SPEC>::State& state, 
            rl::environments::multirotor::observation::ActionHistory<OBSERVATION_SPEC>, 
            Matrix<OBS_SPEC>& observation, 
            RNG& rng)
        {
            using T = typename SPEC::T;
        ...
        RL_TOOLS_FUNCTION_PLACEMENT static void observe(
            ...
            const rl::environments::Multirotor<SPEC>& env, 
            const typename rl::environments::Multirotor<SPEC>::State& state, 
            rl::environments::multirotor::observation::RandomForce<OBSERVATION_SPEC>, 
            Matrix<OBS_SPEC>& observation, 
            RNG& rng)
        {
            using T = typename SPEC::T;
...
//    }
//    template<typename DEVICE, typename T, typename TI, typename SPEC, typename OBS_SPEC, typename RNG>
//    RL_TOOLS_FUNCTION_PLACEMENT static void observe_privileged(DEVICE& device, const rl::environments::Multirotor<SPEC>& env, const rl::environments::multirotor::StateLatentRandomForce<T, TI>& state, Matrix<OBS_SPEC>& observation, RNG& rng){
...
//    RL_TOOLS_FUNCTION_PLACEMENT static void observe_privileged(DEVICE& device, const rl::environments::Multirotor<SPEC>& env, const rl::environments::multirotor::StateBase<T, TI, LATENT_STATE>& state, Matrix<OBS_SPEC>& observation, RNG& rng){
//        using MULTIROTOR = rl::environments::Multirotor<SPEC>;
...
//    RL_TOOLS_FUNCTION_PLACEMENT static void observe(DEVICE& device, const rl::environments::Multirotor<SPEC>& env, const rl::environments::multirotor::StateBaseRotorsHistory<T_H, TI_H, HISTORY_LENGTH, LATENT_STATE>& state, Matrix<OBS_SPEC>& observation, RNG& rng){
//        using MULTIROTOR = rl::environments::Multirotor<SPEC>;
//        static_assert(OBS_SPEC::ROWS == 1);
...
//    RL_TOOLS_FUNCTION_PLACEMENT static void observe_privileged(DEVICE& device, const rl::environments::Multirotor<SPEC>& env, const rl::environments::multirotor::StateBaseRotors<T_S, TI_S, LATENT_STATE>& state, Matrix<OBS_SPEC>& observation, RNG& rng){
//        using MULTIROTOR = rl::environments::Multirotor<SPEC>;
//        using T = typename SPEC::T;
//        using TI = typename DEVICE::index_t;
//        static_assert(OBS_SPEC::ROWS == 1);
...
//    RL_TOOLS_FUNCTION_PLACEMENT void post_integration(DEVICE& device, const rl::environments::Multirotor<SPEC>& env, rl::environments::multirotor::StateBase<T, TI>& state) {
    template<typename DEVICE, typename SPEC, typename ACTION_SPEC, typename T_S, typename TI_S, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT void post_integration(
        DEVICE& device, 
        const rl::environments::Multirotor<SPEC>& env, 
        const typename rl::environments::multirotor::StateBase<T_S, TI_S>& state, 
        const Matrix<ACTION_SPEC>& action, 
        typename rl::environments::multirotor::StateBase<T_S, TI_S>& next_state, 
        RNG& rng) 
    {
        using T = T_S;
        using TI = TI_S;
...
//    RL_TOOLS_FUNCTION_PLACEMENT void post_integration(DEVICE& device, const rl::environments::Multirotor<SPEC>& env, rl::environments::multirotor::StateRotors<T, TI, NEXT_COMPONENT>& state) {
    ...
    RL_TOOLS_FUNCTION_PLACEMENT void post_integration(
        DEVICE& device, 
        const rl::environments::Multirotor<SPEC>& env, 
        const typename rl::environments::multirotor::StateRotors<T_S, TI_S, NEXT_COMPONENT>& state, 
        const Matrix<ACTION_SPEC>& action, 
        typename rl::environments::multirotor::StateRotors<T_S, TI_S, NEXT_COMPONENT>& next_state, 
        RNG& rng) 
    {
        post_integration(
            device, 
            env, 
            static_cast<const NEXT_COMPONENT&>(state), 
            action, 
            static_cast<NEXT_COMPONENT&>(next_state), 
            rng);
        using MULTIROTOR = rl::environments::Multirotor<SPEC>;
        for(typename DEVICE::index_t rpm_i = 0; rpm_i < MULTIROTOR::ACTION_DIM; rpm_i++){
            next_state.rpm[rpm_i] = math::clamp(
                typename DEVICE::SPEC::MATH{}, 
                next_state.rpm[rpm_i], 
                env.parameters.dynamics.action_limit.min, 
                env.parameters.dynamics.action_limit.max);
        }
    }
    template<typename DEVICE, typename SPEC, typename ACTION_SPEC, typename T_S, typename TI_S, typename NEXT_COMPONENT, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT void post_integration(
        DEVICE& device, 
        const rl::environments::Multirotor<SPEC>& env, 
        const typename rl::environments::multirotor::StateRandomForce<T_S, TI_S, NEXT_COMPONENT>& state, 
        const Matrix<ACTION_SPEC>& action, 
        typename rl::environments::multirotor::StateRandomForce<T_S, TI_S, NEXT_COMPONENT>& next_state, 
        RNG& rng) 
    {
        post_integration(
            device, 
    ...
    RL_TOOLS_FUNCTION_PLACEMENT void post_integration(
        DEVICE& device, 
        const rl::environments::Multirotor<SPEC>& env, 
        const typename rl::environments::multirotor::StateRotorsHistory<T_S, TI_S, HISTORY_LENGTH, NEXT_STATE_COMPONENT>& state, 
        const Matrix<ACTION_SPEC>& action, 
        typename rl::environments::multirotor::StateRotorsHistory<T_S, TI_S, HISTORY_LENGTH, NEXT_STATE_COMPONENT>& next_state, 
        RNG& rng) 
    {
        using MULTIROTOR = rl::environments::Multirotor<SPEC>;
        using TI = typename DEVICE::index_t;
        ...
        post_integration(
            device, 
            env, 
            static_cast<const rl::environments::multirotor::StateRotors<T_S, TI_S, NEXT_STATE_COMPONENT>&>(state), 
            action, 
            static_cast<rl::environments::multirotor::StateRotors<T_S, TI_S, NEXT_STATE_COMPONENT>&>(next_state), 
            rng);
...
//    }
    ...
    RL_TOOLS_FUNCTION_PLACEMENT static typename SPEC::T step(
        ...
        const typename rl::environments::Multirotor<SPEC>::State& state, 
        const Matrix<ACTION_SPEC>& action, 
        typename rl::environments::Multirotor<SPEC>::State& next_state, 
        RNG& rng) 
    {
        using STATE = typename rl::environments::Multirotor<SPEC>::State;
        using T = typename SPEC::T;
        ...
        for(TI action_i = 0; action_i < ACTION_DIM; action_i++)
        {
            ...
            action_noisy += random::normal_distribution::sample(
                typename DEVICE::SPEC::RANDOM(), 
                (T)0, 
                env.parameters.mdp.action_noise.normalized_rpm, 
                rng);
...
//            state.rpm[action_i] = action_scaled[action_i];
        ...
        utils::integrators::rk4 
            ...
            STATE, 
            ACTION_DIM, 
            rl::environments::multirotor::multirotor_dynamics_dispatch
                <
                DEVICE, 
                typename SPEC::T, 
                typename SPEC::PARAMETERS, 
                STATE
                >
            >(device, env.parameters, state, action_scaled, env.parameters.integration.dt, next_state);
...
namespace rl_tools
{
    ...
    template<typename DEVICE, typename SPEC, typename ACTION_SPEC, typename RNG>
    RL_TOOLS_FUNCTION_PLACEMENT void log_reward(
        DEVICE& device, 
        const rl::environments::Multirotor<SPEC>& env, 
        const typename rl::environments::Multirotor<SPEC>::State& state, 
        const Matrix<ACTION_SPEC>& action, 
